...
pageentry_t __attribute__((aligned(PAGESIZE)))
    kernel_pagetable[PAGETABLE_NENTRIES];
static pageentry_t __attribute__((aligned(PAGESIZE)))
    kernel_level2_pagetable[PAGETABLE_NENTRIES];

void virtual_memory_init(void) {
    memset(kernel_pagetable, 0, sizeof(pageentry_t) * PAGETABLE_NENTRIES);
    kernel_pagetable[0] = (uintptr_t) kernel_level2_pagetable | PTE_P | PTE_W | PTE_U;
...
void virtual_memory_map(pageentry_t* pagetable, uintptr_t va, uintptr_t pa,
    ...
    int l1pageindex = -1;
    pageentry_t* l2pagetable = 0;
    for (; sz != 0; va += PAGESIZE, pa += PAGESIZE, sz -= PAGESIZE) {
        if (L1PAGEINDEX(va) != l1pageindex) {
            l1pageindex = L1PAGEINDEX(va);
            pageentry_t ptaddr = pagetable[l1pageindex];
            // check that this page table is writable and present
            assert(ptaddr & PTE_P);
            assert(pagetable[L1PAGEINDEX(ptaddr)] & PTE_P);
            l2pagetable = (pageentry_t*) PTE_ADDR(ptaddr);
        }
        if (perm & PTE_P)
            l2pagetable[L2PAGEINDEX(va)] = pa | perm;
        else
            l2pagetable[L2PAGEINDEX(va)] = perm;
    }
}
...
vamapping virtual_memory_lookup(pageentry_t* pagetable, uintptr_t va) {
    pageentry_t pe = pagetable[L1PAGEINDEX(va)];
    if (pe & PTE_P) {
        pageentry_t* l2pagetable = (pageentry_t*) PTE_ADDR(pe);
        pe = l2pagetable[L2PAGEINDEX(va)] & (pe | ~(PTE_W | PTE_U));
    }
    if (pe & PTE_P)
...
void set_pagetable(pageentry_t* pagetable) {
